...
su git -c 'USER=git gitolite setup -a dummy'
...
su git -c "cd repositories/gitolite-admin.git && GL_LIBDIR=$(gitolite query-rc GL_LIBDIR) hooks/post-update refs/heads/master"
